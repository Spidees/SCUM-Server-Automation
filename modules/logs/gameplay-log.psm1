...
function Get-NewGameplayActivities {
    ...
    try {
        # Read all lines from the log file - SCUM gameplay logs use UTF-16 LE encoding
        ...
        if (Test-Path $script:StateFile) {
            ...
            if ($latestLogFile -and (Test-Path $latestLogFile)) {
                ...
                try {
                    ...
                    Write-Log "Initialized gameplay log state: File=$($script:CurrentLogFile), Starting from line $($script:LastLineNumber)" -Level "Info"
                } catch {
                    $script:LastLineNumber = 0
                }
            } else {
