...
function Test-GameProcessHealth {
    ...
    try {
        ...
        try {
                ...
                if ($serviceProcess -and $serviceProcess.ProcessName -eq "nssm") {
                    ...
                    if ($scumChild) {
                        ...
                        if (-not $script:LastKnownScumPid -or $script:LastKnownScumPid -ne $scumProcess.Id) {
                            Write-Log "[Service] Found SCUM process via NSSM: $($scumProcess.ProcessName) PID $($scumProcess.Id)" -Level Verbose
            ...
            Write-Log "[Service] Could not get process from service: $($_.Exception.Message)" -Level Verbose
        }
        ...
        if (Get-Command "Test-DatabaseConnection" -ErrorAction SilentlyContinue) {
            try {
                ...
                Write-Log "[Service] Database connectivity check failed: $($_.Exception.Message)" -Level Verbose
            }
        }
        ...
        if ($ServerDirectory) {
            ...
            if (Test-Path $logPath) {
                try {
                    ...
                    Write-Log "[Service] Could not check log file: $($_.Exception.Message)" -Level Verbose
                }
            }
        }
